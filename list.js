...
function render(items){const enriched=new Set(window.NIKE_ENRICHED||[]);let enrichedBannerShown=false;grid.innerHTML='<div class="section-banner">Standard PDP experience demo</div>'+items.map((p,index)=>{let banner='';if(!enrichedBannerShown&&enriched.has(p.id)){banner='<div class="section-banner section-banner-enriched">Enriched PDP experience demo</div>';enrichedBannerShown=true}return banner+`<a class="product-card" data-product="${p.id}" href="index.html?product=${encodeURIComponent(p.id)}&from=list"><div class="photo"><img src="${p.image}" alt="${p.shortTitle}"></div><div class="product-copy"><h2 class="product-title">${p.shortTitle}</h2><p class="subline">${p.subtitle}</p><div class="price-row">¥<strong>${p.price}</strong><small>补贴后</small><span class="sold">${p.sold}</span></div><div class="meta-shot"><img src="assets/list-meta-${index%2?'right':'left'}.png" alt="限时红包、小黑盒新品和购物车" /></div></div></a>`}).join('')}
render(products);
